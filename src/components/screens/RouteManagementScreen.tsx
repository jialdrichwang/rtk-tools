...
import { TrackRecordingMap } from '../common/TrackRecordingMap';
...
export const RouteManagementScreen: React.FC<RouteManagementScreenProps> = ({
  ...
  // Track Recording Map state
  const [isMapExpanded, setIsMapExpanded] = useState(false);
  const [focusedTrack, setFocusedTrack] = useState<SurveyTrack | null>(null);
    ...
    <div className="flex-1 flex flex-col bg-[#F1F5F9] text-slate-800 overflow-hidden select-none">
      ...
      {activeTab === 'record' && (
        <div className="flex-1 p-3.5 overflow-y-auto space-y-3">
          {/* Active Highlighted Track Banner if inspecting a recorded track from files */}
          {focusedTrack && (
            <div className="bg-amber-50 border border-amber-300 rounded-xl px-3 py-2 text-xs flex items-center justify-between text-amber-900 shadow-2xs">
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-amber-600 shrink-0" />
                <span className="text-[11px] leading-tight">
                  底图正在高亮查阅已存航迹: <b>{focusedTrack.name}</b> ({focusedTrack.points.length} 点, {focusedTrack.distance}m)
                </span>
              </div>
              <button
                type="button"
                onClick={() => setFocusedTrack(null)}
                className="px-2 py-0.5 rounded bg-amber-200 hover:bg-amber-300 text-amber-800 font-bold text-[10px] cursor-pointer shrink-0 ml-2"
              >
                清除高亮
              </button>
            </div>
          ...
          {/* Interactive Live Track Recording Map with Multi-layer Support */}
          <TrackRecordingMap
            currentLat={rtkState.currentLat}
            currentLon={rtkState.currentLon}
            currentAlt={rtkState.currentAlt}
            heading={rtkState.heading}
            speed={rtkState.speed}
            hrms={rtkState.hrms}
                    ...
                    <div className="flex items-center justify-between pt-1 text-xs">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="px-2 py-1 rounded bg-teal-50 hover:bg-teal-100 text-teal-700 text-[11px] font-bold border border-teal-200 flex items-center gap-1 cursor-pointer transition shadow-2xs"
                          title="在航迹记录底图中预览高亮"
                        >
